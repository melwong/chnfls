		...
		if ( $order_id ) {		
			...
			if ( $amount > 0 ) {			
				if ( 'offline' == $gateway ) {
					...
					//Mel: 27/01/22
					acadp_order_completed( array( 'id' => $order_id, 'transaction_id' => $tx_hash, 'user_wallet' => $user_wallet, 'token_uri' => $token_uri, 'gas_used' => $gas_used, 'block_number' => $block_number, 'contract_address' => $contract_address ) );
