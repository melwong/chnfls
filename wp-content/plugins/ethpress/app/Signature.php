...
class Signature
{
    ...
    public static function extract_address_with_php( $message, $signature )
    {
        ...
        $recovered_address = '0x' . substr( Keccak256::hash( substr( hex2bin( $pub_key->encode( 'hex' ) ), 1 ), 256 ), 24 );
    ...
    public static function extract_address_with_api( $message, $signature )
    {
        ...
        if ( is_null( $api_url ) ) {
            ...
            if ( 'https://api.ethereumico.io/ethpress' === $options['api_url'] ) {
                error_log( "extract_address_with_api failed for bad api_url" );
                return false;
            }
        ...
        error_log( "extract_address_with_api for {$message}, {$signature}: api_url = {$api_url}" );
        $data = [
            'message'   => $message,
            'signature' => $signature,
        ];
        ...
        if ( !empty($body) ) {
            error_log( "extract_address_with_api for {$message}, {$signature}: res = {$body}" );
            $body = json_decode( $body );
            return $body->data;
        } else {
            error_log( "extract_address_with_api failed for {$message}, {$signature}: res is empty for {$api_url} and args=" . print_r( $args, true ) );
            return false;
        }
    ...
    public static function verify( $message, $signature, $coinbase )
    {
        ...
        if ( empty($signature) || empty($coinbase) || !is_string( $signature ) || !is_string( $coinbase ) ) {
            error_log( "verify failed for: {$message}, {$signature}, {$coinbase}" );
            return false;
        }
        ...
        if ( !is_string( $address ) ) {
            error_log( "verify failed for: {$message}, {$signature}, {$coinbase}: address is not a string" );
            return false;
        }
        
        if ( strtolower( $coinbase ) !== strtolower( $address ) ) {
            error_log( "verify failed for: {$message}, {$signature}, {$coinbase}, {$address}: address !== coinbase" );
        }
